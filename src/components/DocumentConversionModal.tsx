...
interface DocumentConversionModalProps {
    isOpen: boolean;
    onClose: () => void;
    mode: 'quote-to-order' | 'order-to-ack' | 'rfq-to-quote';
    triggerToast: (title: string, description: string, type: 'success' | 'error' | 'info') => void;
    /** When provided, the modal skips the 'select' step and opens directly at 'compare' with that doc preselected. */
...
const ELIGIBLE_QUOTES: DocItem[] = [
    // Post-Neocon-review (2026-06-05) · E.1.i taxonomy: Quote statuses In Progress / Pending / Sent / Expired
    // (Approved becomes a PO; Negotiating and Draft removed).
    { id: 'QT-1020', customer: 'Summit Healthcare', project: 'Clinic Furniture', amount: '$68,200', status: 'Sent', date: 'Dec 15, 2025', initials: 'SH' },
    { id: 'QT-1022', customer: 'Redwood School District', project: 'Classroom Refresh', amount: '$150,000', status: 'Sent', date: 'Dec 28, 2025', initials: 'RS' },
    { id: 'QT-1025', customer: 'Apex Furniture', project: 'New HQ Buildout', amount: '$43,750', status: 'Pending', date: 'Jan 12, 2026', initials: 'AF' },
    { id: 'QT-1028', customer: 'FinServe Corp', project: 'Branch Rollout', amount: '$890,000', status: 'Sent', date: 'Jan 08, 2026', initials: 'FS' },
    { id: 'QT-1033', customer: 'BioLife Inc', project: 'Lab Expansion', amount: '$540,000', status: 'In Progress', date: 'Jan 10, 2026', initials: 'BL' },
...
const ELIGIBLE_ORDERS: DocItem[] = [
    ...
    { id: '#ORD-2055', customer: 'AutoManfacture Co.', project: 'Office Renovation', amount: '$385,000', status: 'PO received', date: 'Dec 20, 2025', initials: 'AC' },
    ...
    { id: '#ORD-2051', customer: 'City Builders', project: 'City Center Phase 2', amount: '$120,000', status: 'PO received', date: 'Jan 05, 2026', initials: 'CB' },
    { id: '#ORD-2049', customer: 'Meridian Group', project: 'Regional Office', amount: '$98,400', status: 'Shipped', date: 'Feb 10, 2026', initials: 'MG' },
];
...
const RFQ_COMPARISON: ComparisonField[] = [
    ...
    { field: 'Line 1: List Price', category: 'pricing', docValue: '$1,485.00', sourceValue: '$1,485.00', status: 'match' },
    { field: 'Line 2: Finish (Conference Table)', category: 'line-item', docValue: 'Walnut', sourceValue: 'Walnut Veneer - Natural', status: 'partial', autoFixSuggestion: 'Map dealer finish "Walnut" to manufacturer SKU CT-WVN-NAT (Walnut Veneer - Natural).', confidence: 95, severity: 'low' },
    { field: 'Line 3: Qty (Task Chair)', category: 'line-item', docValue: '32', sourceValue: '32', status: 'match' },
    { field: 'Line 3: List Price', category: 'pricing', docValue: '$685.00', sourceValue: '$685.00', status: 'match' },
    { field: 'Discount Structure', category: 'pricing', docValue: '60% / 5%', sourceValue: '60% / 5%', status: 'match' },
    { field: 'Requested Delivery', category: 'logistics', docValue: 'Apr 01, 2026', sourceValue: 'Apr 14, 2026', status: 'partial', autoFixSuggestion: 'Current lead time is 9 weeks. Apr 14 is earliest available — within 21-day guardrail of dealer request.', confidence: 92, severity: 'medium' },
    { field: 'Freight Terms', category: 'logistics', docValue: 'FOB Origin', sourceValue: 'FOB Origin', status: 'match' },
    { field: 'Total Estimated', category: 'pricing', docValue: '$67,240.00', sourceValue: '$67,240.00', status: 'match' },
];
...
export default function DocumentConversionModal({ isOpen, onClose, mode, triggerToast, preselectedDocId }: DocumentConversionModalProps) {
    // Compute initial preselected doc from props; defensive lookup against current mode's documents.
    const preselectedDoc = useMemo(() => {
        if (!preselectedDocId) return null;
        const pool = mode === 'quote-to-order' ? ELIGIBLE_QUOTES
            : mode === 'rfq-to-quote' ? ELIGIBLE_RFQS
            : ELIGIBLE_ORDERS;
        return pool.find(d => d.id === preselectedDocId) ?? null;
    }, [mode, preselectedDocId]);

    const [step, setStep] = useState<ConversionStep>(preselectedDoc ? 'compare' : 'select');
    const [selectedDoc, setSelectedDoc] = useState<DocItem | null>(preselectedDoc);
    ...
    const config = useMemo(() => {
        if (mode === 'quote-to-order') {
            return {
                ...
                newId: '#ORD-2060',
                newStatus: 'Order Entered',
            };
        }
        if (mode === 'rfq-to-quote') {
            return {
                title: 'Convert RFQ to Quote',
                sourceLabel: 'RFQ',
                targetLabel: 'Quote',
                TargetIcon: DocumentTextIcon,
                eligibleStatuses: ['Pending Review', 'In Review'],
                documents: ELIGIBLE_RFQS,
                comparison: RFQ_COMPARISON,
                newId: 'QT-1026',
                newStatus: 'Draft',
            };
        }
        return {
            ...
            eligibleStatuses: ['Ready', 'PO received'],
    ...
    // Reset on close — respect preselected state so re-opening from a detail page jumps back to compare
    useEffect(() => {
        if (!isOpen) {
            const t = setTimeout(() => {
                setStep(preselectedDoc ? 'compare' : 'select');
                setSelectedDoc(preselectedDoc);
                setScanProgress(0);
                                        ...
                                        <div className="flex justify-between items-start mb-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[11px] text-muted-foreground">Against manufacturer database & external sources</p>
                                                </div>
                                            </div>
                                            <button onClick={onClose} className="p-2 rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
                                                <XMarkIcon className="w-5 h-5" />
                                            ...
                                            <div>
                                                ...
                                                <span className="text-sm text-indigo-700 dark:text-indigo-300">
                                                    {scanComplete
                                                        ...
                                                        : ` — Scanning ${selectedDoc?.id} against manufacturer database & external sources...`
                                                    }
                                                </span>
                                            </div>
                                        </div>
                                                    ...
                                                    <table className="w-full text-sm">
                                                        <thead>
                                                            <tr className="bg-muted/50">
                                                                ...
                                                                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Purchase Order</th>
                                            ...
                                            {discrepancies.map((disc, i) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {/* Action buttons.
                                                            Post-Neocon-review (2026-06-05) · L.7 + L.10 + L.19: manufacturer DETECTS, doesn't RESOLVE.
                                                            Accept Fix / Keep Original removed — replaced by a single Notify dealer button so the
                                                            exception goes back to the dealer who is the actual decision-maker (Wendy + Christian 38:01). */}
                                                        {fixed === undefined && (
                                                            <div className="flex items-center gap-2">
                                                                <button
                                                                    onClick={() => setDiscrepancyFixes(prev => ({ ...prev, [i]: 'accept' }))}
                                                                    title="Drafts an email to the dealer with this exception · they decide the resolution"
                                                                ...
                                                                >
                                                                    ...
                                                                    Notify dealer of exception
                                                                </button>
                                                            </div>
                                                        )}
                                                        {fixed !== undefined && (
                                                            <div className="flex items-center gap-2">
                                                                <span className={`text-[11px] font-semibold ${fixed === 'accept' ? 'text-info' : 'text-muted-foreground'}`}>
                                                                    {fixed === 'accept' ? 'Dealer notified · awaiting reply' : 'Dismissed'}
                                                                </span>
                                                                <button
                                    ...
                                    <div className="p-6 sm:p-8 text-center">
                                        ...
                                        <div className="p-3 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 mb-6 flex items-start gap-3 text-left max-w-md mx-auto">
                                            ...
                                            <span className="text-[11px] text-indigo-700 dark:text-indigo-300">
                                                {isCleanDoc
                                                    ? `All ${matchedCount} fields validated — zero discrepancies. Direct conversion completed and synced to internal ERP.`
                                                    : `${discrepancies.length} discrepancies resolved. ${acceptedCount} auto-fixed, ${discrepancies.length - acceptedCount} kept as original. All changes synced to internal ERP.`
                                                }
                                            </span>
                                        </div>
